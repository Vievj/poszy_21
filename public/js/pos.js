...
function pos_product_row(variation_id = null,
                         purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
...
function storeValue() {
    checker.input_val.val($('#weight_machine').val()).trigger('change');
...
async function usbConnect() {
    ...
    if ('serial' in navigator) {
        checker.input = $('#weight_machine');

        if (checker.port) {

            try {
                await checker.port.open({
                    baudRate: 9600,
                    dataBits: 8,
                    // bufferSize: 2048,
                    stopBits: 1,
                ...
                checker.ports = await navigator.serial.getPorts();
                if ((checker.ports !== null) && (Array.isArray(checker.ports))) {
                    checker.port = checker.ports[0];
                } else {
...
async function getReader() {
    ...
    checker.appendStream = new WritableStream({
        write(chunk) {
            if (chunk.length >= 6) {
                // console.log('Value > 6', chunk);
                checker.input.val(parseFloat(chunk).toFixed(3));
    ...
    // console.log('port append ', checker.appendStream)
    ...
    try {
        ...
        // alert('you must reload your browser cuz there is a changes @ the serial port');
    }
